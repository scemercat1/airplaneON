...
class General(commands.Cog):
    ...
    # ======= THE MISSING REACTION ROLE LISTENER LINK =======
    @commands.Cog.listener()
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        # Prevent the bot from triggering its own role assignments
        ...
        # Ensure we are handling an active message generated by our reaction panel system
        if not msg.embeds or not msg.footer.text or "Aircraft Games - Role System" not in msg.footer.text:
            return
        ...
        # Read the message embed fields to see which roles match our emojis dynamically
        embed = msg.embeds[0]
        if not embed.fields:
            return
        ...
        # Parse out the correct role mention corresponding to the selected emoji
        for line in field_value.split("\n"):
            if line.startswith(emoji_str):
                # Clean up format strings like "1️⃣ - <@&1234567890>" to isolate the raw ID bits
                try:
                    role_id_str = line.split("<@&")[1].replace(">", "").strip()
