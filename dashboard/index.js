...
app.get("/dashboard", (req, res) => {
    ...
    res.send(`
        <!DOCTYPE html>
        <html>
        <head><title>Aircraft Dashboard</title><style>body{background:#0f0f0f;color:white;font-family:sans-serif;padding:40px}.card{background:#1a1a1a;padding:20px;border-radius:10px;max-width:500px;margin-bottom:20px}input,select{width:100%;padding:10px;margin:10px 0;background:#2a2a2a;color:white;border:none;border-radius:5px}button{background:#5865F2;color:white;border:none;padding:10px 20px;border-radius:5px;cursor:pointer}</style></head>
        <body>
            <h1>Aircraft Dashboard</h1>
            <div class="card"><h3>Server</h3><select id="guild">${options}</select></div>
            <div class="card"><h3>Messages</h3><input id="warn" placeholder="Warn {reason}"><input id="kick" placeholder="Kick {reason}"><button onclick="save()">Save</button></div>
            <script>async function save(){await fetch("/api/save",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({guild:document.getElementById("guild").value,warn:document.getElementById("warn").value,kick:document.getElementById("kick").value})});alert("Saved!");}</script>
        </body></html>`);
});

app.post("/api/save", async (req, res) => {
    await settings_col.updateOne({ guild_id: req.body.guild }, { $set: { warn: req.body.warn, kick: req.body.kick } }, { upsert: true });
    res.json({ ok: true });
});
